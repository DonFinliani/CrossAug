...
def cache_response(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        key_data = {
            ...
            "extra_kwargs": {key: value for key, value in kwargs.items() if key != "messages"},
        }
...
class CacheOpenAI(BaseLLM):
    ...
    def from_experiment_config(cls, global_config: BaseConfig) -> "CacheOpenAI":
        ...
        cache_dir = getattr(global_config, "llm_cache_dir", None) or os.path.join(global_config.save_dir, "llm_cache")
    ...
    def __init__(self, cache_dir, global_config, cache_filename: str = None,
        ...
        if high_throughput:
            ...
            client = httpx.Client(limits=limits, timeout=httpx.Timeout(10*60, read=10*60))
        else:
            client = None
        ...
        if self.global_config.azure_endpoint is None:
            api_key = getattr(self.global_config, "llm_api_key", None)
            client_kwargs = {
    ...
    def infer(
        ...
        **kwargs
        ...
        if ('gpt' not in params['model'] or version.parse(openai.__version__) < version.parse("1.45.0")) and 'max_completion_tokens' in params: # if we use vllm to call openai api or if we use openai but the version is too old to use 'max_completion_tokens' argument
