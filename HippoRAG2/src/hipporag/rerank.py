...
class DSPyFilter:
    ...
    def parse_filter(self, response, query=None, fact_before_filter=None):
        sections = [(None, [])]
        ...
        for k, value in sections:
            if k == "fact_after_filter":
                try:
                    ...
                    print(
                        "Rerank parse debug info:\n"
                        f"Question:\n```\n{query}\n```\n\n"
                        f"Fact before filter:\n```\n{fact_before_filter}\n```\n\n"
                        f"Raw rerank response:\n```\n{response}\n```"
                    )
    ...
    def llm_call(self, question, fact_before_filter):
        ...
        self.default_gen_kwargs['max_completion_tokens'] = 5120

        result = self.llm_infer_fn(
            messages=messages,
    ...
    def rerank(self,
        ...
        try:
            ...
            response, metadata = self.llm_call(query, json.dumps(fact_before_filter))
            generated_facts = self.parse_filter(response, query=query, fact_before_filter=json.dumps(fact_before_filter))
        except Exception as e:
            print('exception', e)
