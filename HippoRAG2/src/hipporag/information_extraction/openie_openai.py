...
class OpenIE:
    ...
    def ner(self, chunk_key: str, passage: str) -> NerRawOutput:
        ...
        try:
            raw_response, metadata, cache_hit = self.llm_model.infer(
                ...
                seed=getattr(self.llm_model.global_config, "seed", 42),
                temperature=getattr(self.llm_model.global_config, "temperature", 0),
            )
            ...
            # 替换ellipsis为字符串"..."，保证可len()和可json序列化
            processed_entities = [
                "..." if (e is ... or isinstance(e, type(...))) else e
                for e in extracted_entities
            ]
            unique_entities = list(dict.fromkeys(processed_entities))
    ...
    def batch_openie(self, chunks: Dict[str, ChunkInfo]) -> Tuple[Dict[str, NerRawOutput], Dict[str, TripleRawOutput]]:
        ...
        num_cache_hit = 0
        max_workers = max(1, int(getattr(self.llm_model.global_config, "llm_concurrency", 32)))
