...
LEGACY_TARGETS = {
    "gfmrag.models.gfm_rag_v1.GNNRetriever": "gfmrag.models.GNNRetriever",
    "gfmrag.models.gfm_rag_v1.QueryGNN": "gfmrag.models.QueryGNN",
    "gfmrag.models.gfm_rag_v1.rankers.IDFWeightedTopKRanker": (
        "gfmrag.doc_rankers.IDFWeightedTopKRanker"
    ),
    "gfmrag.models.gfm_rag_v1.rankers.IDFWeightedRanker": (
        "gfmrag.doc_rankers.IDFWeightedRanker"
    ),
    "gfmrag.models.gfm_rag_v1.rankers.TopKRanker": "gfmrag.doc_rankers.TopKRanker",
    "gfmrag.models.gfm_rag_v1.rankers.SimpleRanker": "gfmrag.doc_rankers.SimpleRanker",
    "gfmrag.models.ultra.models.QueryNBFNet": "gfmrag.ultra.models.QueryNBFNet",
    "gfmrag.models.ultra.models.EntityNBFNet": "gfmrag.ultra.models.EntityNBFNet",
}


def _normalize_pretrained_config(config: dict) -> dict:
    """Normalize old pretrained configs to the current package layout."""

    def normalize_targets(value):
        if isinstance(value, dict):
            normalized = {
                key: normalize_targets(item) for key, item in value.items()
            }
            target = normalized.get("_target_")
            if isinstance(target, str) and target in LEGACY_TARGETS:
                normalized["_target_"] = LEGACY_TARGETS[target]
            return normalized
        if isinstance(value, list):
            return [normalize_targets(item) for item in value]
        return value

    normalized_config = normalize_targets(config)
    model_config = normalized_config.get("model_config")
    if isinstance(model_config, dict):
        if "rel_emb_dim" not in model_config and "feat_dim" in model_config:
            model_config["rel_emb_dim"] = model_config.pop("feat_dim")
        # Older checkpoints saved ranker settings inside the model config, but the
        # current retriever builds document rankers outside of the torch module.
        model_config.pop("ranker", None)
...
def load_model_from_pretrained(path: str) -> tuple[torch.nn.Module, dict]:
    ...
    with open(config_path) as f:
        config = json.load(f)
    config = _normalize_pretrained_config(config)
