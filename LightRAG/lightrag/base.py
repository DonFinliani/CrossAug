...
class QueryParam:
    ...
    gnn_context_gate: Literal[
        "off",
        "require_final_evidence_chunk",
        "require_final_evidence_chunk_new_only",
    ] = os.getenv("GNN_CONTEXT_GATE", "off")
    """Controls query-time filtering for GNN-augmented relationship context.

    - "off": keep all retrieved relationships.
    - "require_final_evidence_chunk": keep a GNN-affected relationship only if
      at least one of its GNN evidence chunks survives into the final chunk context.
    - "require_final_evidence_chunk_new_only": apply the same gate only to
      newly inserted GNN edges; merged preexisting edges are kept because the
      base and GNN descriptions cannot be separated after graph merge.
    """

    gnn_context_gate_artifact_dir: str | None = None
    """Optional directory or report path used to locate gnn_injection_report_gnn.json."""
