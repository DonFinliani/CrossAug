...
async def _summarize_descriptions(
    ...
    try:
        ...
        if global_config.get("skip_failed_llm_calls") and is_timeout_exception(exc):
            logger.warning(
                "Skipping timed-out summary LLM call for %s `%s`; using joined descriptions",
                description_type,
                description_name,
            )
...
async def extract_entities(
    ...
    chunk_results = []
    if global_config.get("skip_failed_llm_calls"):
        results = await asyncio.gather(*tasks, return_exceptions=True)
        skipped_chunks = []
        first_exception = None

        for (chunk_key, _chunk_dp), result in zip(ordered_chunks, results):
            if isinstance(result, BaseException):
                if is_timeout_exception(result):
                    skipped_chunks.append((chunk_key, result))
                    logger.warning(
                        "Skipping timed-out entity extraction chunk `%s`: %s",
                        chunk_key,
                        result,
                    )
                    continue
        ...
        if skipped_chunks:
            log_message = (
                f"Skipped {len(skipped_chunks)} timed-out chunks during entity "
                f"extraction; continuing with {len(chunk_results)}/{total_chunks} chunks"
            )
...
async def kg_query(
    ...
    # Handle cache. The primary key is context-aware, so GNN/base queries only
    # share QA cache when the actual LLM input is identical.
    legacy_args_hash = compute_args_hash(
        query_param.mode,
        query,
        query_param.response_type,
        query_param.top_k,
    ...
    if cached_result is None and global_config.get(
        "allow_legacy_query_cache_fallback", True
    ):
        cached_result = await handle_cache(
            hashing_kv,
            legacy_args_hash,
        ...
        legacy_cache_hit = cached_result is not None

    if cached_result is not None:
        cached_response, _ = cached_result  # Extract content, ignore timestamp
        if legacy_cache_hit:
            logger.info(
                " == LLM cache == Legacy query cache hit, migrating to context-aware query cache"
            )
        else:
            logger.info(
                " == LLM cache == Context-aware query cache hit, using cached response as query result"
            )
        ...
        if (
            ...
            await save_to_cache(
                ...
                CacheData(
                    ...
                    cache_type="query",
                    queryparam={
                        "cache_version": "query-v2-context-aware",
                        "legacy_args_hash": legacy_args_hash,
                        "mode": query_param.mode,
...
def _relation_pair_key(src: Any, tgt: Any) -> tuple[str, str] | None:
    src_text = str(src or "").strip()
    tgt_text = str(tgt or "").strip()
    if not src_text or not tgt_text:
        return None
    return tuple(sorted((src_text, tgt_text)))
...
def _split_graph_source_ids(source_id: Any) -> list[str]:
    ...
    if isinstance(source_id, list):
        values = [str(item or "").strip() for item in source_id]
    else:
        values = split_string_by_multi_markers(str(source_id), [GRAPH_FIELD_SEP])
    output: list[str] = []
    seen: set[str] = set()
    for value in values:
...
def _resolve_gnn_injection_report_path(
    query_param: QueryParam,
    global_config: dict[str, Any],
) -> Path | None:
    explicit_path = getattr(query_param, "gnn_context_gate_artifact_dir", None)
    if explicit_path:
        path = Path(explicit_path)
        if path.is_dir():
            return path / "gnn_injection_report_gnn.json"
        return path

    working_dir = global_config.get("working_dir")
    if not working_dir:
        return None
    return Path(str(working_dir)) / "gnn_subgraph_completion" / "gnn_injection_report_gnn.json"


def _load_gnn_injection_index(
    query_param: QueryParam,
    global_config: dict[str, Any],
) -> tuple[dict[tuple[str, str], list[dict[str, Any]]], dict[str, Any]]:
    report_path = _resolve_gnn_injection_report_path(query_param, global_config)
    ...
    try:
        ...
        logger.warning("Failed to load GNN injection report %s: %s", report_path, exc)
        return {}, metadata

    if not isinstance(report_items, list):
        metadata["load_error"] = "report is not a list"
        logger.warning("Invalid GNN injection report format: %s", report_path)
        return {}, metadata

    index: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    success_statuses = {"inserted_new_edge", "merged_existing_edge", "dry_run"}
    for item in report_items:
        ...
        completed_relation = item.get("completed_relation") or {}
        src = (
            relation.get("src_id")
            or completed_relation.get("source_entity")
            or completed_relation.get("src_id")
        )
        tgt = (
            relation.get("tgt_id")
            or completed_relation.get("target_entity")
            or completed_relation.get("tgt_id")
        )
        key = _relation_pair_key(src, tgt)
        if key is None:
            continue

        evidence_chunk_ids = _split_graph_source_ids(relation.get("source_id"))
        evidence_chunk_ids.extend(
            _split_graph_source_ids(completed_relation.get("evidence_chunk_ids"))
        )
        evidence_chunk_ids = list(dict.fromkeys(evidence_chunk_ids))
        preexisting_edge = bool(
            relation.get("preexisting_edge") or status == "merged_existing_edge"
        )
        index[key].append(
            {
                "status": status,
                "preexisting_edge": preexisting_edge,
                "evidence_chunk_ids": evidence_chunk_ids,
                "missing_score": completed_relation.get("missing_score"),
                "subgraph_root": completed_relation.get("subgraph_root"),
            }
        )

    metadata["report_found"] = True
    metadata["report_items"] = len(report_items)
    metadata["indexed_relations"] = sum(len(items) for items in index.values())
    return dict(index), metadata


def _apply_gnn_context_gate(
    ...
    mode = getattr(query_param, "gnn_context_gate", "off") or "off"
    gate_metadata: dict[str, Any] = {
        "mode": mode,
        "enabled": mode != "off",
        "gnn_relations_seen": 0,
        "gnn_relations_checked": 0,
        "gnn_relations_kept": 0,
        "gnn_relations_dropped": 0,
        "gnn_preexisting_relations_kept_without_gate": 0,
        "final_chunk_count": len(final_chunks),
        "dropped_relation_keys": [],
    }

    if mode == "off":
        return relations_context, relation_id_to_original or {}, gate_metadata

    valid_modes = {
        "require_final_evidence_chunk",
        "require_final_evidence_chunk_new_only",
    }
    if mode not in valid_modes:
        logger.warning("Unknown gnn_context_gate mode `%s`; disabling gate", mode)
        gate_metadata["enabled"] = False
        gate_metadata["error"] = f"unknown mode: {mode}"
        return relations_context, relation_id_to_original or {}, gate_metadata

    gnn_index, report_metadata = _load_gnn_injection_index(query_param, global_config)
    gate_metadata.update(report_metadata)
    if not gnn_index:
        return relations_context, relation_id_to_original or {}, gate_metadata
    ...
    kept_relation_id_to_original: dict = {}
    relation_id_to_original = relation_id_to_original or {}
    dropped_relation_keys: list[str] = []

    for relation in relations_context:
        src = relation.get("entity1")
        tgt = relation.get("entity2")
        directional_key = (src, tgt)
        normalized_key = _relation_pair_key(src, tgt)
        gnn_records = gnn_index.get(normalized_key) if normalized_key else None
        if not gnn_records:
            kept_relations.append(relation)
            if directional_key in relation_id_to_original:
                kept_relation_id_to_original[directional_key] = relation_id_to_original[
                    directional_key
                ]
            continue

        gate_metadata["gnn_relations_seen"] += 1
        if mode == "require_final_evidence_chunk_new_only" and all(
            record.get("preexisting_edge") for record in gnn_records
        ):
            gate_metadata["gnn_preexisting_relations_kept_without_gate"] += 1
            kept_relations.append(relation)
            if directional_key in relation_id_to_original:
                kept_relation_id_to_original[directional_key] = relation_id_to_original[
                    directional_key
        ...
        gate_metadata["gnn_relations_checked"] += 1
        evidence_chunk_ids = {
            chunk_id
            for record in gnn_records
            for chunk_id in record.get("evidence_chunk_ids", [])
        }
        if evidence_chunk_ids & final_chunk_ids:
            gate_metadata["gnn_relations_kept"] += 1
            kept_relations.append(relation)
            if directional_key in relation_id_to_original:
                kept_relation_id_to_original[directional_key] = relation_id_to_original[
                    directional_key
            ...
            gate_metadata["gnn_relations_dropped"] += 1
            if len(dropped_relation_keys) < 50 and normalized_key is not None:
                dropped_relation_keys.append(f"{normalized_key[0]}||{normalized_key[1]}")

    gate_metadata["dropped_relation_keys"] = dropped_relation_keys
    if gate_metadata["gnn_relations_dropped"]:
        logger.info(
            "GNN context gate `%s`: kept %d, dropped %d, skipped preexisting %d",
            mode,
            gate_metadata["gnn_relations_kept"],
            gate_metadata["gnn_relations_dropped"],
            gate_metadata["gnn_preexisting_relations_kept_without_gate"],
        )
    return kept_relations, kept_relation_id_to_original, gate_metadata
...
async def _build_context_str(
    ...
    relations_context, relation_id_to_original, gnn_gate_metadata = (
        _apply_gnn_context_gate(
            relations_context=relations_context,
            relation_id_to_original=relation_id_to_original,
            final_chunks=truncated_chunks,
            query_param=query_param,
            global_config=global_config,
        )
    )
    ...
    if "metadata" not in final_data:
        final_data["metadata"] = {}
    final_data["metadata"]["gnn_context_gate"] = gnn_gate_metadata
    logger.debug(
...
async def _build_query_context(
    ...
    raw_data["metadata"]["processing_info"] = {
        ...
        "final_relations_count": len(raw_data.get("data", {}).get("relationships", [])),
    }
...
async def naive_query(
    ...
    # Handle cache. The primary key includes the full system prompt, including
